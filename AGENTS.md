...
- `WezTermIntegration` writes `~/.config/wezterm/tuicode.lua` and appends a fenced `-- TuiCode begin / -- TuiCode end` block to the user's `wezterm.lua` (creating it if absent). The block calls `tuicode.apply(config)`, which registers a `tuicode` key table and a `user-var-changed` handler. `WorkbenchHost` emits OSC 1337 `SetUserVar=TUICODE_ACTIVE=1` on startup (after `_app.Init`) and `=0` on shutdown (post-`_app.Dispose`, pre-`_flowControl.Dispose`) to push/pop the key table — unconditional, since terminals that don't grok OSC 1337 strip it silently. Staleness marker (`-- TuiCodeIntegrationVersion: N`) lives in both the module and the fenced block. tmux passthrough caveat: nested sessions need `set -g allow-passthrough on` for the OSC to reach WezTerm.
