...
- Windows Authenticode signing auto-enables when `SIGNPATH_API_TOKEN` exists; without it the `win-*` zips ship unsigned (SmartScreen warns on first launch). Cloud signing via [SignPath](https://signpath.org/) (Foundation plan, free for OSS) — the `win-*` legs upload the unsigned `TuiCode.exe` as an `unsigned-<rid>` artifact, `signpath/github-action-submit-signing-request` signs it in the cloud, and the signed binary drops back into `publish/` before Archive. No key material touches the runner. The release job's `download-artifact` filters `pattern: tuicode-*` so the `unsigned-*` signing inputs never reach the release. Config (non-secret) lives in repo **variables** `SIGNPATH_ORGANIZATION_ID` / `SIGNPATH_PROJECT_SLUG` / `SIGNPATH_SIGNING_POLICY_SLUG` / `SIGNPATH_ARTIFACT_CONFIGURATION_SLUG`; the API token is the lone **secret**. SignPath origin-verifies the request against the repo via its GitHub App, so the signing policy must trust this repo/workflow.
