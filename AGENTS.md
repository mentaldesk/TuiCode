...
- Theme: Dark+ or Light+, chosen per frame by `EditorTextView.IsDarkScheme` from the Editable attribute, so switching TG theme re-colours without any settings plumbing. TG's Dark theme leaves the background to the terminal (`Color.None`), so it judges by text colour there. A switch bumps `SyntaxHighlighter.ThemeVersion`, which makes every cache re-lex (token metadata encodes theme colour ids). TG's Default theme draws the editor on Gray, where Dark+ colours have modest contrast.
